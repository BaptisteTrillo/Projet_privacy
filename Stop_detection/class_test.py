...
class stop_move_detection:
    ...
    def segmented_data(self,participant):
        
        df_hilbert = stop_hilbert_vgp(participant_virtual_id = participant)
        ...
        activities = [dfs_hilbert[i].activity.unique()[0] for i in range(len(dfs_hilbert))]
        stops = [dfs_hilbert[i].stop.median() for i in range(len(dfs_hilbert))]
        _activity_ = [dfs_hilbert[i]._activity_.median() for i in range(len(dfs_hilbert))]
        _stops_ = [dfs_hilbert[i]._stops_.median() for i in range(len(dfs_hilbert))]
        ...
        #moves.to_csv('..\..\TrajLib\data\moves_'+str(self.participant_virtual_id)+'.csv', index=False)
    ...
    def get_features(self):
        ...
        ts_obj.load_data(lat='lat',lon='lon',time_date='time', labels=['activity'],src='.\data_record\moves_'+str(self.participant_virtual_id)+'.csv',seperator=',')
        ...
        for seg in range(len(dfs)):
            ...
            if(dfs[seg].shape[0]>5):
                tr_obj=tr.Trajectory(mood='df',trajectory=dfs[seg],labels=['activity'])
        ...
        features_set.to_csv('data/features_participant_'+str(self.participant_virtual_id)+'.csv')
